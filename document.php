...
$Supplier__SupDoc_portal = fmsRelatedRecord($Supplier_row, 'SupDoc');
...
<title>SpecTrak | Add New Document</title>
...
var uploadOrAssign = '';
var allAssigned = 0;
var anyDocs = '<?php echo $Supplier__SupDoc_portal->getField('SupDoc::_PrimeDocIDX'); ?>';
...
var deleteAssignedText = 'This document is currently assigned to other SKUs and cannot be deleted. You must de-assign it from all SKUs before it can be deleted.';
var deleteResponseText = 'The document has been deleted';
...
$(document).ready(function(){
	...
	if(deleteResponse == 'a'){
		setTimeout(alert(deleteAssignedText), 500);
	} else if (deleteResponse == 'd'){
		setTimeout(alert(deleteResponseText), 500);
	}
	
	// CHECK FOR AND DOCUMENTS
	if(anyDocs == ''){
		$("#noDocInst").show();
		$("#cmp_items").hide();
	...
	if (infoReturned || infoAssigned_item){
		...
		if(infoReturned){
			if(infoPassFail == "pass"){
				...
				flashNotice("NOTICE: New document creation failed", infoReturned);
			}
...
function saveAction(){
	...
	if(uploadOrAssign == 'upload'){
		...
		if(errorLog){
			flashNotice("NOTICE: There were issues saving this document", errorLog);
			highlightErrors();
			if(errorFileSize){
	...
	if(uploadOrAssign == 'assign'){
		if(assignedCheckItems != ''){
			$('#upload_and_assign').attr('action', 'document_assign_response.php');
			myForm.submit();
		} else {
			flashNotice("NOTICE: There were issues assigning this document", "You must assign the selected document to at least one SKU to complete assignment.");
			highlightErrors();
		}
	}
}
...
function deleteItem(item){
	...
	var urlString = 'document_delete.php?item=' + item;
	if(skus == ''){
...
<ul id="actions" class="MenuBarVertical">
    ...
    <li id="add_another"><a href="<?php echo $_SERVER['REQUEST_URI']; ?>">Add Another Document</a></li>
...
<!-- INSTRUCTIONS -->
    <span id="instructions">
    	<p><span class="callout">Instruction:</span> Select an existing document from the list below, it will highlight in blue. To add a new document, click on the + button.<br>Select the SKU or SKUs that use that document, they will also highlight in blue. When you are finished, click <em>Save</em> in the side menu or at the bottom of the page.<br><strong>NOTE:</strong> SKUs that have a status of &quot;Submitted&quot; or &quot;Complete&quot; cannot be edited. Contact Galileo Global Branding Group, Inc. if you need to access these SKUs.</p>
    </span> 
...
<!-- FORM --> 
    <form action="document_new_response.php" method="post" enctype="multipart/form-data" name="upload_and_assign" id="upload_and_assign">
  
<!-- UPLOAD DETAIL TABLE -->
    <table id="item_upload_table" class="table" width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td class="table_header">Document</td>
        ...
        <script>
			$("ul:last").qtip({ 
				show: 'mouseover',
				hide: 'mouseout',
				content: 'Show <?php echo $Supplier_row->getField('cmpCompany'); ?> Documents' ,
				style: { 
      ...
      <tr>
        ...
        <td class="table_row"><span id="sprytextfield2">
          <input type="text" name="docDescription" id="docDescription">
          </span><span id="new_item_description"></span></td>
      </tr>
      <tr>
        <td class="table_side_head_req">Accepted Filetypes:<br />(Adobe Illustrator, EPS, Microsoft Excel, Microsoft Word, PDF, Plain Text, Rich Text Format, SITX, ZIP)<br /><br /><strong>MAXIMUM FILE SIZE: 30MB</strong></td>
        <td class="table_row">
...
<div id="cmp_items">
	<!-- HEADER DIV -->
    <div class="DivHeadToggle"><?php echo $Supplier_row->getField('cmpCompany'); ?> Documents
    	<!-- ADD BTN -->
    ...
    <div id="itemList" class="table_list">
      <?php
$Supplier__SupDoc_portal_rownum = 1;
foreach(fmsRelatedSet($Supplier_row,'SupDoc') as $Supplier__SupDoc_portal_row=>$Supplier__SupDoc_portal){ 

$timestamp = explode(" ", $Supplier__SupDoc_portal->getField('SupDoc::docZdevCreationTimestampAuto'));
$time = strtotime( $timestamp[0] );
$creationDate = date( 'F j, Y', $time );
$itemName = $Supplier__SupDoc_portal->getField('SupDoc::docFileName');
$itemID = $Supplier__SupDoc_portal->getField('SupDoc::_PrimeDocIDX');
$active = $Supplier__SupDoc_portal->getField('SupDoc::DocActiveFlag');
if($active != 1){ $class = "hidden"; } else { $class = ""; }

?>
        <div id="itemContainer" value="<?php echo $Supplier__SupDoc_portal->getField('SupDoc::_PrimeDocIDX'); ?>" onClick="toggleITEM(this)" class="<? echo $class; ?>">
          <div id="itemDeleteRight" title="Delete <? echo $itemName; ?>" onclick="setTimeout( 'deleteItem(<?php echo $itemID; ?>)', 200 )"></div>
		  <div id="itemViewRight" onclick="openDocument('<?php echo $Supplier__SupDoc_portal->getField('SupDoc::docPath'); ?>')"></div>
          <div id="itemUnchecked" name="<?php echo $nameEditable; ?>"></div>
          <div id="itemChecked" name="<?php echo $nameEditable; ?>"></div>
		  <div id="itemName" name="itemName"><?php echo $Supplier__SupDoc_portal->getField('SupDoc::docFileName'); ?></div>
        </div>
        <?php if($Supplier__SupDoc_portal_rownum == 0) break; else $Supplier__SupDoc_portal_rownum++;
}//portal_end ?>
    </div>
    <div id="noDocInst" class="table_row_instr"><?php echo $Supplier_row->getField('cmpCompany'); ?> has no Documents yet</div>
</div>
  ...
  <div id="upcList" class="table_list">
	...
	<?php
		...
		$upc_FK_doc = $Job__JobUpc_portal->getField('JobUpc::upc_FK_doc');
		$item_PK_Formatted = "/\b" . $newItem_PK . "\b/";
		$filtered_PK = preg_match($item_PK_Formatted, $upc_FK_doc);
		$items = "/" . str_replace("\n", "/", $Job__JobUpc_portal->getField('JobUpc::upc_FK_doc') ) . "/";
  ...
  <input name="assigned_upc" type="hidden" value="" size="50" id="assigned_upc">
  <input name="assigned_item" type="hidden" value="" size="50" id="assigned_item">
  <input name="job_upc_pk" type="hidden" value="" size="50" id="job_upc_pk">
  ...
  <input name="doc_FK_cmp" type="hidden" value="<?php echo $Supplier_row->getField('cmp_PK'); ?>" id="doc_FK_cmp">
