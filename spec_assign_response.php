...
$spec_pk = $_REQUEST['assigned_item'];
...
{ 
	...
	$item_upc_fk_spe_arr = explode("\n", $UPC_row->getField('upc_FK_spe'));
	$spec_pk_arr = array($spec_pk);
	$new_upc_fk_spe = "";
	$assignment = FALSE;
	...
	//echo "<b>item_upc_fk_spe_arr:</b> ";
	//print_r($item_upc_fk_spe_arr);
	//echo "<br>";
	if(in_array($item_upc_pk, $upc_pk_arr)) { 										// upc on the list to be assigned
	
		if (in_array($spec_pk, $item_upc_fk_spe_arr)) { 							// spec already assigned
			//$new_upc_fk_spe = implode("\r", $item_upc_fk_spe_arr);
			//echo "already assigned<br>";
		} else {																	// add spec to assigned list
			array_push($item_upc_fk_spe_arr, $spec_pk);
			$new_upc_fk_spe = implode("\r", $item_upc_fk_spe_arr);
			$assignment = TRUE;
		...
		if (in_array($spec_pk, $item_upc_fk_spe_arr)) {								// remove spec from assigned list
			
			$spec_key = array_search($spec_pk, $item_upc_fk_spe_arr);
			$assignment = TRUE;
			//echo "needs to be removed<br>";
			//echo "spec_key: ";
			//print_r($spec_key);
			unset($item_upc_fk_spe_arr[$spec_key]);
			$new_upc_fk_spe = implode("\r", $item_upc_fk_spe_arr);
			//echo "<br>new string: " . $new_upc_fk_spe;
			//echo $item_upc_fk_spe_arr[$spec_key];
	...
	//echo "<b>new_upc_fk_spe:</b> " . $new_upc_fk_spe . "<br><br>";
	
	// UPDATE UPC
	if($assignment){
		
		$EDIT_UPC_edit = $ContactLogin->newEditCommand('sup-UPC-assign',$item_upc_recid);
		$EDIT_UPC_fields = array('upc_FK_spe'=>$new_upc_fk_spe,'upcPDKStatus'=>$PDKstatus,);
...
$errorVar = 'Printer Specification assigned';
$passFail = 'pass';
...
<title>Update Specification Assignments</title>
...
<form id="returnInfo" name="returnInfo" method="post" action="<?php echo $redirect; ?>">
  ...
  <input id="assigned_item" name="assigned_item" type="hidden" value="<?php echo $spec_pk ?>">
</form>
</body>
</html>
