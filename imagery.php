...
<title>SpecTrak | Add New Imagery</title>
...
var uploadOrAssign = '';
var allAssigned = 0;
var anyItems = '<?php echo $Supplier__SupImg_portal->getField('SupImg::_PrimeImgIDX'); ?>';
...
var maxuploadFileSize = 31457280;
...
var deleteAssignedText = 'This imagery is currently assigned to other SKUs and cannot be deleted. You must de-assign it from all SKUs before it can be deleted.';
var deleteResponseText = 'The imagery has been deleted';
	...
	if (infoReturned || infoAssigned_item){
		...
		if(infoReturned){
			if(infoPassFail == "pass"){
				...
				flashNotice("NOTICE: New imagery creation failed", infoReturned);
			}
...
function saveAction(){
	...
	if(uploadOrAssign == 'upload'){
		...
		if(SS == false || !assignedCheckUPC) {
			errorLog = errorLog + "You must assign the new image to at least one SKU to complete assignment. Please enter valid data into the fields highlighted in red.";
		}
		if(errorLog){
			flashNotice("NOTICE: There were issues saving this image", errorLog);
			highlightErrors();
			if(errorFileSize){
	...
	if(uploadOrAssign == 'assign'){
		if(assignedCheckItems != ''){
			$('#upload_and_assign').attr('action', 'imagery_assign_response.php');
			myForm.submit();
		} else {
			flashNotice("NOTICE: There were issues assigning this image", "You must assign the selected image to at least one SKU to complete assignment.");
			highlightErrors();
		}
	}
...
function deleteItem(item){
	...
	var urlString = 'imagery_delete.php?item=' + item;
	if(skus == ''){
...
<!-- INSTRUCTIONS -->
    <span id="instructions">
    	<p><span class="callout">Instruction:</span> Select an existing image from the list below, it will highlight in blue. To add a new image, click on the + button.<br>Select the SKU or SKUs that use that image, they will also highlight in blue. When you are finished, click <em>Save</em> in the side menu or at the bottom of the page.<br><strong>NOTE:</strong> SKUs that have a status of &quot;Submitted&quot; or &quot;Complete&quot; cannot be edited. Contact Galileo Global Branding Group, Inc. if you need to access these SKUs.</p>
    </span>
...
<!-- FORM --> 
    <form action="imagery_new_response.php" method="post" enctype="multipart/form-data" name="upload_and_assign" id="upload_and_assign">
  
<!-- UPLOAD DETAIL TABLE -->
    <table id="item_upload_table" class="table" width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td class="table_header">Imagery</td>
        ...
        <script>
			$("ul:last").qtip({ 
				show: 'mouseover',
				hide: 'mouseout',
				content: 'Show <?php echo $Supplier_row->getField('cmpCompany'); ?> Imagery' ,
				style: { 
      ...
      <tr>
        <td class="table_side_head_req">Electronic Imagery<br /><br />High Resolution Image Files (TIFF, PSD, EPS JPG)<br /><br /><strong>MAXIMUM FILE SIZE: 30MB</strong></td>
        <td class="table_row">
...
<div id="cmp_items">
	<!-- HEADER DIV -->
    <div class="DivHeadToggle"><?php echo $Supplier_row->getField('cmpCompany'); ?> Imagery
    	<!-- ADD BTN -->
    ...
    <div id="itemList" class="table_list">
      <?php
$Supplier__SupImg_portal_rownum = 1;
foreach(fmsRelatedSet($Supplier_row,'SupImg') as $Supplier__SupImg_portal_row=>$Supplier__SupImg_portal){ 

$timestamp = explode(" ", $Supplier__SupImg_portal->getField('SupImg::ImgZdevCreationTimestampAuto'));
$time = strtotime( $timestamp[0] );
$creationDate = date( 'F j, Y', $time );
$itemName = $Supplier__SupImg_portal->getField('SupImg::ImgFileName');
$itemID = $Supplier__SupImg_portal->getField('SupImg::_PrimeImgIDX');
$active = $Supplier__SupImg_portal->getField('SupImg::ImgActiveFlag');
if($active != 1){ $class = "hidden"; } else { $class = ""; }

?>
        <div id="itemContainer" value="<?php echo $Supplier__SupImg_portal->getField('SupImg::_PrimeImgIDX'); ?>" onClick="toggleITEM(this)" class="<? echo $class; ?>">
          <div id="itemDeleteRight" title="Delete <? echo $itemName; ?>" onclick="setTimeout( 'deleteItem(<?php echo $itemID; ?>)', 200 )"></div>
		  <div id="itemViewRight" onclick="openDieline('<?php echo $Supplier__SupImg_portal->getField('SupImg::ImgPath'); ?>')"></div>
          <div id="itemUnchecked" name="<?php echo $nameEditable; ?>"></div>
          <div id="itemChecked" name="<?php echo $nameEditable; ?>"></div>
          <div id="itemNameLeft" name="item"><?php echo $Supplier__SupImg_portal->getField('SupImg::ImgFileName'); ?></div>
          <div name="itemName" id="itemName"><?php echo $creationDate; ?></div>
        </div>
        <?php if($Supplier__SupImg_portal_rownum == 0) break; else $Supplier__SupImg_portal_rownum++;
}//portal_end ?>
    </div>
    <div id="noItemInst" class="table_row_instr"><?php echo $Supplier_row->getField('cmpCompany'); ?> has no Imagery yet</div>
</div>
  ...
  <div id="upcList" class="table_list">
	...
	<?php
		...
		$items = "/" . str_replace("\n", "/", $Job__JobUpc_portal->getField('JobUpc::upc_FK_img') ) . "/";
